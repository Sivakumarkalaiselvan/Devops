# Web App scripts
locals{
web_app_vmss_custom_data= <<CUSTOM_DATA
#!/bin/sh
#sudo yum update -y
...
sudo echo "Welcome to my westus Web VMSS" > /var/www/html/default.html
CUSTOM_DATA
}
...
resource "azurerm_linux_virtual_machine_scale_set" "web_vmss"{
    ...
    computer_name_prefix = "Web-Vmss-App1"                        # Optional - Host Name
    ...
    custom_data = base64encode(local.web_app_vmss_custom_data)
    ...
    network_interface {
      ...
      ip_configuration {
        ...
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.web_backend_pool.id]      # Associating Web VMSS with LB
        load_balancer_inbound_nat_rules_ids = [azurerm_lb_nat_pool.web_vmss_nat.id]
      }
    }
}
...
resource "null_resource" "move_ssh_key_to_web_vmss"{
...
connection {
  type = "ssh"
  host = azurerm_public_ip.web_lb_pub_ip.ip_address
  port = 5000
  user = azurerm_linux_virtual_machine_scale_set.web_vmss.admin_username
