# Web App scripts
locals{
app1_vmss_custom_data= <<CUSTOM_DATA
#!/bin/sh
#sudo yum update -y
...
sudo echo "Welcome to my App1 VMSS" > /var/www/html/app1
CUSTOM_DATA
}
...
resource "azurerm_linux_virtual_machine_scale_set" "app1_vmss"{
    name = "${local.resource_name_prefix}-app1-vmss"
    computer_name_prefix = "app1-Vmss-App1"                        # Optional - Host Name
    ...
    custom_data = base64encode(local.app1_vmss_custom_data)
    upgrade_mode = "Automatic"

    network_interface {
      name = "app1-web-vmss-nic"
      primary = true
      network_security_group_id = azurerm_network_security_group.app1_vmss_nsg.id
      ip_configuration {
        name = "internal-1"
        primary = true
