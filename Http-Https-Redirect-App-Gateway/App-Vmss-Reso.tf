# Web App scripts
locals{
app_vmss_custom_data= <<CUSTOM_DATA
#!/bin/sh
#sudo yum update -y
...
sudo echo "Welcome to my App VMSS" > /var/www/html/default.html
CUSTOM_DATA
}
...
resource "azurerm_linux_virtual_machine_scale_set" "app_vmss"{
    ...
    computer_name_prefix = "app-Vmss-App1"                        # Optional - Host Name
    ...
    custom_data = base64encode(local.app_vmss_custom_data)
